...
The sustainability of open-source software (OSS) is a critical concern as modern society increasingly relies on these projects as a digital foundation\cite{sample}. However, research has shown that most OSS projects suffer from a high concentration of knowledge, relying on a very small group of ``core developers''\cite{sample}. 

To quantify this risk, the ``Truck Factor'' (TF) metric—also known as the Bus Factor—was proposed to represent the minimum number of developers who must leave a project before it becomes incapacitated\cite{sample}. Empirical studies on popular GitHub repositories have revealed that 65\% of these projects have a Truck Factor of two or less\cite{sample}. Furthermore, a large-scale study of over 36,000 projects found that 89\% of projects have experienced the loss of their core development team at least once, with 70\% of these abandonments occurring within the first three years of the project's life\cite{sample}. Only 27\% of projects that lose their core team are able to survive and attract new core developers\cite{sample}.

The danger of this structural vulnerability is best illustrated by high-profile security incidents. The ``Log4Shell'' vulnerability in the Log4j library exposed how critical global infrastructure was maintained by uncompensated volunteers, some of whom worked 22-hour shifts to mitigate the crisis\cite{sample}. Similarly, the Heartbleed incident in OpenSSL revealed that a project securing the majority of the internet was operating on an annual donation budget of only US\$2,000, with only one full-time developer\cite{sample}. These cases highlight the urgent need for external funding to support core developers and ensure long-term sustainability\cite{sample}.

\subsection{Funding in Open-source Ecosystems}
In response to sustainability challenges, platforms like Open Collective have emerged to provide a transparent mechanism for soliciting and managing donations\cite{sample}. Research into these platforms distinguishes between two primary types of donors: individuals and corporations\cite{sample}. While corporate donors tend to contribute larger amounts in single transactions, individual donors often provide more total funds to a collective over time and are more likely to make recurring donations\cite{sample}.

The way these donations are utilized is diverse. A study of 225 GitHub projects found that non-engineering-related expenses—such as web services, marketing, and travel—account for 54\% of the total number of expenses\cite{sample}. Within engineering-related expenses, funds are most frequently spent on development, maintenance, and bounties for fixing specific issues\cite{sample}. While some evidence suggests that monthly donation amounts are positively associated with maintenance activities in corporate-supported collectives, there is still limited knowledge regarding how specific types of expenditures impact overall software development productivity\cite{sample}.
